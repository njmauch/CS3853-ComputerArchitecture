...
file_names = ['A-9_new_1.5.pdf.trc', 'Trace2A.trc']
cache_sizes = [8, 64, 256, 1024]
block_sizes = [4, 16, 64]
rep_polices = ['RR', 'RND']
associativities = [1, 2, 4, 8, 16]
csv_file = "output.csv"
header = ['Cache Size (in KB)', 'Block Size (in bytes)', 'Associativity', 'Replacement', '# Blocks', '# Rows', 'Overhead', 'Imp Memory Size', 'Cost in $', 'Total cache accesses', 'Cache hits', 'Cache Misses', 'Comp Misses', 'Conf Misses', 'Hit Rate', 'Miss Rate', 'CPI', 'Unused Cache Space', '% Unused Cache Space', 'Waste in $', 'Unused Cache Blocks']
    ...
    for c_size in cache_sizes:
        for b_size in block_sizes:
            for assoc in associativities:
                for r_polices in rep_polices:
                    #print("\nTrace File: ",sys.argv[2])
                    #print("\n***** Cache Input Parameters *****\n")
                    #print("Cache Size:\t\t\t",c_size, " kb")
                    #print("Block Size:\t\t\t",b_size, " bytes")
                    #print("Associativity:\t\t\t",assoc)
                    rep_policy = r_polices
                    #if(rep_policy == "RR"):
                    ...
                    block_size = b_size
                    associativity = assoc
                    cache_bits = math.ceil(Log2(c_size * 1000))
                    offset = math.ceil(Log2(b_size))
                    ...
                    row = [c_size, b_size, assoc, r_polices, block_count, total_rows, overhead, (total_impl * 0.0009765625), cost, total, hit, (comp_miss + conf_miss), comp_miss, conf_miss, hit_rate, (100 - hit_rate), (cycles/instruction), unused_cache, ((unused_cache/total_cache_size)*100), waste, unused_cache_blocks]
